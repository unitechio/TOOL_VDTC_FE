  ...
  */
  ...
  const handleSubmit = () => {
    ...
    if (startTime === "" || endTime === "" || startTime > endTime) {
      ...
      axios
        .get(
          `${urlBase}/api/cskh/sms-toi-uu?startTime=${startTime}&endTime=${endTime}&optionTool=${optionTool}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
              "Content-Type": "multipart/form-data",
  ...
  return (
    <div>
      ...
      <div className="container">
        ...
        Báo cáo Đăng ký gói SMS tối ưu
        </h2>
        <div>
          Start Date:{" "}
          <input
        ...
        <div id="download-button-sale-order">
          {result ? (
            <div>
              <p>Click down below to download BaoCaoSms.XLSX</p>
              <button
                className="btn btn-info"
                onClick={() => handleDownload(result)}
              >
                Download BaoCaoSms.XLSX
              </button>
            </div>
          ) : (
            ""
