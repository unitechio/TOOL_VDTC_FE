  ...
  */
  ...
  const handleSubmit = () => {
    ...
    if (startTime === "" || endTime === "" || startTime > endTime) {
      ...
      axios
        .get(
          `${urlBase}/api/gsvh/contract-termination?startTime=${startTime}&endTime=${endTime}&optionTool=${optionTool}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
              "Content-Type": "multipart/form-data",
  ...
  return (
    <div>
      ...
      <div className="container">
        <h2 className="text-center text-primary m-3">Danh sách hủy hợp đồng</h2>
        <div>
          Start Date:{" "}
          <input
            type="date"
        ...
        <div id="download-button-sale-order">
          {result ? (
            <div>
              <p>Click down below to download ContractTermination.XLSX</p>
              <button
                className="btn btn-info"
                onClick={() => handleDownload(result)}
              >
                Download ContractTermination.XLSX
              </button>
            </div>
          ) : (
            ""
...
export default CustTermination;
