  ...
  */
  ...
  const searchContract = () => {
    ...
    if (
      ...
      axios
        .get(
          `${urlBase}/api/gsvh/danh-sach-hoa-don?startTime=${startTime}&endTime=${endTime}&contractNo=${contractNo}&optionTool=${optionTool}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
              "Content-Type": "multipart/form-data",
    ...
    <div>
      ...
      <div className="container">
        <h2 className="text-center text-primary m-3">
          Danh sách hóa đơn theo hợp đồng
        </h2>
        <div>
          Start Date:{" "}
          <input
        ...
        <div className="row">
          <div id="download-button-sale-order">
            {result ? (
              <div>
                <p>Click down below to download HoaDon.XLSX</p>
                <button
                  className="btn btn-info"
                  onClick={() => handleDownload(result)}
                >
                  Download HoaDon.XLSX
                </button>
              </div>
            ) : (
              ""
