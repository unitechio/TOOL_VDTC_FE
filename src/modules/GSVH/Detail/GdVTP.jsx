  ...
  */
  ...
  const handleSubmit = () => {
    ...
    if (date === "" ) {
      ...
      axios
        .get(
          `${urlBase}/api/gsvh/giao-dich-lien-ket-vtp?date=${date}&optionTool=${optionTool}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
              "Content-Type": "multipart/form-data",
  ...
  return (
    <div>
      ...
      <div className="container">
        ...
        Giao dịch liên kết VTP
        </h2>
        <div className="mt-5">
        Ngày lấy Danh sách giao dịch liên kết VTP:
          <input
            type="date"
            className="ms-3 rounded me-3 mb-3"
            style={{ fontSize: "25px" }}
            onChange={(e) => {
        ...
        <div id="download-button-sale-order">
           {result?<div>
           <p>Click down below to download VTP-Transaction.XLSX</p>
            <button className="btn btn-info" onClick={() => handleDownload(result)}>
            Download VTP-Transaction.XLSX
            </button>
           </div>:""}
           </div>
