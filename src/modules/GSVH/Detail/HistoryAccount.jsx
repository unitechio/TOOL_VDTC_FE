...
function HistoryAccount() {
  ...
  */
  ...
  const handleInput = (e) =>  {setContractNo(e.target.value);
  };
  ...
  const searchContract = () => {
    ...
    const isValid = regex.test(contractNo);
    if (!isValid || contractNo.length !=10) {
      setError("Nhập đúng định dạng!!!");
    } else {
        setShowLoading(true)
      axios
        .get(
          `${urlBase}/api/gsvh/lich-su-thay-doi-tai-khoan?contractNo=${contractNo}&optionTool=${optionTool}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
              "Content-Type": "multipart/form-data",
  ...
  return (
    <div>
      ...
      <div className="container" >
        <h2 className="text-center text-primary m-3">Lịch sử thay đổi tài khoản theo hợp đồng</h2>
        <div >
          Mã hợp đồng: 
          <input
        ...
        <div id="download-button-sale-order">
           {result?<div>
           <p>Click down below to download LichSuTaiKhoan.XLSX</p>
            <button className="btn btn-info" onClick={() => handleDownload(result)}>
            Download LichSuTaiKhoan.XLSX
            </button>
           </div>:""}
          </div>
        </div>
