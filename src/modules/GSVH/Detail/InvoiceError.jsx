  ...
  */
  ...
  const handleSubmit = () => {
    ...
    if (startTime === "" || endTime === "" || startTime > endTime) {
      ...
      axios
        .get(
          `${urlBase}/api/gsvh/invoice-error-info?startTime=${startTime}&endTime=${endTime}&optionTool=${optionTool}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
              "Content-Type": "multipart/form-data",
  ...
  return (
    <div>
      ...
      <div className="container">
        ...
        Lịch sử hóa đơn lỗi do thông tin KH
        </h2>
        <div>
          Start Date:{" "}
          <input
        ...
        <div id="download-button-sale-order">
          {result ? (
            <div>
              <p>Click down below to download InvoiceErrorInfo.XLSX</p>
              <button
                className="btn btn-info"
                onClick={() => handleDownload(result)}
              >
                Download InvoiceErrorInfo.XLSX
              </button>
            </div>
          ) : (
            ""
