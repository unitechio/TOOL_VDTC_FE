  ...
  */
  ...
  const handleSubmit = () => {
    ...
    if (startTime === "" || endTime === "" || startTime>endTime) {
      ...
      axios
        .get(
          `${urlBase}/api/gsvh/contract_report?startTime=${startTime}&endTime=${endTime}&optionTool=${optionTool}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
              "Content-Type": "multipart/form-data",
    ...
    <div>
      ...
      <div className="container">
        <h2 className="text-center text-primary m-3">
          Báo cáo nạp tiền theo hợp đồng
        </h2>
        <div>
          Start Date:{" "}
          <input
        ...
        <div id="download-button-sale-order">
          {result ? (
            <div>
              <p>Click down below to download ContractReport.XLSX</p>
              <button
                className="btn btn-info"
                onClick={() => handleDownload(result)}
              >
                Download ContractReport.XLSX
              </button>
            </div>
          ) : (
            ""
