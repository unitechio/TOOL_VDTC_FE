  ...
  */
  ...
  const handleSubmit = () => {
    ...
    if (startTime === "" || endTime === "" || startTime > endTime) {
      ...
      axios
        .get(
          `${urlBase}/api/gsvh/khdn-not-vehicle?startTime=${startTime}&endTime=${endTime}&optionTool=${optionTool}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
              "Content-Type": "multipart/form-data",
    ...
    <div>
      ...
      <div className="container">
        <h2 className="text-center text-primary m-3">
          Thông tin khách hàng doanh nghiệp chưa có xe
        </h2>
        <div>
          Start Date:{" "}
          <input
        ...
        <div id="download-button-sale-order">
          {result ? (
            <div>
              <p>Click down below to download KhdnNotVehicle.XLSX</p>
              <button
                className="btn btn-info"
                onClick={() => handleDownload(result)}
              >
                Download KhdnNotVehicle.XLSX
              </button>
            </div>
          ) : (
            ""
